...
int sumSubarrayMins(vector<int> &arr)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        long long count = (leftCount * rightCount); // total subarrays where arr[i] is the minimum
        long long contribution = (count * arr[i]);  // sum of the minimums where arr[i] is the minimum and multiply it with total subarrays where arr[i] is the minimum

        ans = (ans + contribution); // add the contribution to the final answer with modulo
    }

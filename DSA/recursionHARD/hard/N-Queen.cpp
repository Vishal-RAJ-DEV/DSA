...
// Optimized Approach
//time complexity : o(n!)  as in each row we are placing one queen and for each row we have n choices for first row , n-1 for second row , n-2 for third row and so on
//space complexity : o(n) for leftrow + o(2n -1) for upper diagonal + o(2n -1) for lower diagonal + o(n^2) for board
class Solution {
public:

    ...
    vector<vector<int>> ZigZagLevelOrder(Node* root){
        ...
        while(!nodesQueue.empty()){
            ...
            for(int i = 0; i < size; i++){
                ...
                int index = leftToRight ? i : (size - 1 - i); //for eg if size is 4 and i is 0 then index will be 3 if i is 1 then index will be 2 and so on
                ...
                if(node->left){
                    nodesQueue.push(node->left); //for further levels
                }
                if(node->right){
                    nodesQueue.push(node->right); //for further levels
                }
            }
